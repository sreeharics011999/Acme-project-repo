...
import { AlertModal, AppText, Header } from "../components";
...
const ProfileScreen = ({ navigation }) => {
  ...
  return (
    <View style={{ backgroundColor: colors.primarylight2, flex: 1 }}>
      ...
      <View style={{ marginTop: HEIGHT * 0.07, paddingVertical: HEIGHT * 0.01, alignItems: "center" }}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <Image source={logoutIcon} style={{ width: WIDTH * 0.05, height: WIDTH * 0.05, marginRight: WIDTH * 0.03 }} resizeMode="contain" tintColor={colors.grey} />
          <TouchableOpacity onPress={() => setVisible(true)}>
            <AppText style={{ fontSize: RFValue(16), fontWeight: '700', color: colors.error }}>Log out</AppText>
          </TouchableOpacity>
        </View>
      </View>
      <AlertModal visible={visible} title='Are you sure to Logout ?' btntext2='Logout' setVisible={setVisible} pressConfirm={() => handleLogout()} />
    </View>
  );
};
